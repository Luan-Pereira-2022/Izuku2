...
//Anti ViewOnce Otomatis
  ...
  let teks = `╭「 *Anti-visualização Única* 」\n├ *Nome* : ${pushname}\n├ *Usuário* : @${m.sender.split("@")[0]}\n├ *Hora* : ${wib} WIB\n└ *Mensagem* : ${m.mtype}`
  xcaa.sendMessage(m.chat, { text: teks, mentions: [m.sender] }, { quoted: m })
  await sleep(500)
  m.copyNForward(m.chat, true, {readViewOnce: true}, {quoted: m}).catch(_ => m.reply('Talvez o barco tenha sido aberto antes'))
  }
...
case 'listgc': {
  ...
  let teks = `╭「 *LISTA GRUPOS* 」\n└ Total Grupos : ${anu.length} Grupo\n\n`
  for (let i of anu) {
  let metadata = await xcaa.groupMetadata(i)
      teks += `╭ *Nome :* ${metadata.subject}\n├ *Dono Grupo :* ${metadata.owner !== undefined ? '@' + metadata.owner.split`@`[0] : 'Desconhecido'}\n├ *ID :* ${metadata.id}\n├ *Feito :* ${moment(metadata.creation * 1000).tz('Asia/Jakarta').format('DD/MM/YYYY HH:mm:ss')}\n└ *Membros :* ${metadata.participants.length}\n\n──────────────────\n\n`
  }
  xcaa.sendTextWithMentions(m.chat, teks, m)
  } catch (err) {
  m.reply(mess.error)
...
case 'apagar': case 'del': case 'd': {
  ...
  if (!isBaileys) return m.reply('A mensagem não foi enviada por um bot!')
...
case 'request': case 'reqfitur': case 'reqf': {
  if (!args.join(" ")) return m.reply(`Exemplo : ${prefix + command} Adicionar Recurso De Download`)
  try {
  teks = `╭「 *Solicitação De Recurso* 」\n├ Número : @${m.sender.split("@")[0]}\n└ Solicitar : ${args.join(" ")}`
...
case 'report': case 'lapor': {
  if (!args.join(" ")) return m.reply(`Exemplo : ${prefix + command} Baixe O Recurso De Erro De Reprodução`)
  try {
...
case 'bcgc': case 'bcgroup': {
  ...
  if (!text) return m.reply(`*Exemplo : ${prefix + command} Texto*`)
...
case 'chat': {
  if (!isOwner && !m.key.fromMe) return m.reply(mess.botOwner)
  if (!text) return m.reply('*Exemplo : 1. mute\n2. unmute\n3. archive\n4. unarchive\n5. read\n6. unread\n7. delete*')
  try {
  if (args[0] === 'mute') {
...
case 'group': {
  ...
  await xcaa.sendButtonText(m.chat, buttonsgroup, `Modo Abrir/Fechar Grupo️`, `Por Favor Clique No Botão Abaixo, Se O Botão Não Aparecer, Digite ${command} open/close`, m)
  }
  } catch (err) {
  m.reply(mess.error)
  }
...
case 'editinfo': {
  ...
  await xcaa.sendButtonText(m.chat, buttons, `Modo De Edição De Informações Do Grupo`, `Por Favor Clique No Botão Abaixo, Se O Botão Não Aparece, Digita ${command} open/close`, m)
  }
  } catch (err) {
  m.reply(mess.error)
  }
...
case 'tomp3': {
  ...
  let { toAudio } = require('../lib/converter')
  let audio = await toAudio(media, 'mp4')
  xcaa.sendMessage(m.chat, {document: audio, mimetype: 'audio/mp4', fileName: `Converter por ${xcaa.user.name}.mp3`}, { quoted : m })
  }
  break
...
case 'lindo': {
  ...
  break
case 'pinterest': {
  if (!text) return m.reply(`*Exemplo : ${prefix + command} Naruto*`)
  m.reply(mess.wait)
  try {
  let anu = await pinterest(text)
  result = anu[Math.floor(Math.random() * anu.length)]
  let buttonspinterest = [{buttonId: `${prefix + command} ${text}`, buttonText: {displayText: 'Próxima imagem'}, type: 1}]
  xcaa.sendMessage(m.chat, { image: { url: result }, caption: 'Link da imagen : '+result, buttons: buttonspinterest }, { quoted: m })
  } catch (err) {
  m.reply(mess.error)
  }
  }
  break
case 'wallpaper': {
  if (!text) return m.reply(`*Exemplo : ${prefix + command} Naruto HD*`)
  m.reply(mess.wait)
  try {
...
case 'quotesanime': {
  ...
  result = anu[Math.floor(Math.random() * anu.length)]
  let buttonsquotes = [{buttonId: `${prefix + command}`, buttonText: {displayText: 'Próximas Cotações'}, type: 1}]
  ...
  break
case 'wikimedia': {
  if (!text) throw 'Título da consulta'
  try {
  let wiki = await wikimedia(text)
  result = wiki[Math.floor(Math.random() * wiki.length)]
  let buttons = [{buttonId: `${prefix + command} ${text}`, buttonText: {displayText: 'Próximo Wiki'}, type: 1}]
  let buttonMessage = {
...
case 'couple': case 'ppcp': {
  ...
  let buttons = [{buttonId: `${prefix + command}`, buttonText: {displayText: 'Próxima imagem'}, type: 1}]
  let buttonMessage = {
...
case 'play': case 'ytplay': {
  if (!text) return m.reply(`*Exemplo : ${prefix + command} Biu Do Piseiro*`)
  try {
...
case 'ytmp4': case 'ytvideo': case 'ytv': {
  if (!text) return m.reply(`*Exemplo : ${prefix + command} a ligação 360p*`)
  if (!isUrl(text)) return m.reply('*Link inválido*')
  if (!text.includes('youtube')/('youtu.be')) return m.reply('*Link inválido*')
...
case 'ytmp3': case 'ytaudio': case 'yta': {
  if (!text) return m.reply(`*Exemplo : ${prefix + command} a ligação 128kbps*`)
  if (!isUrl(text)) return m.reply('*Link inválido*')
  if (!text.includes('youtube')/('youtu.be')) return m.reply('*Link inválido*')
  await m.reply(mess.wait)
  try {
...
case 'twitter': case 'twdl': case 'twmp4': {
  if (!args[0]) return m.reply(`*Exemplo : ${prefix + command} a ligação*`)
  try {
...
case 'get': {
  if (!args[0]) return m.reply(`*Exemplo : ${prefix + command} a ligação*`)
  if (!isUrl(text)) return m.reply(`*Exemplo : ${prefix + command} a ligação*`)
  try {
...
case 'translate': case 'terjemahan': {
  if (!args.join(" ")) return m.reply(`*Exemplo : ${prefix + command} Texto*`)
  try {
  ...
  break
case 'volumereq': {
  if (!args.join(" ")) return m.reply(`*Exemplo : ${prefix + command} 10*`)
...
case 'addmsg': {
  ...
  if (!text) return m.reply(`*Exemplo : ${prefix + command} Nome do arquivo*`)
  ...
  break
case 'getmsg': {
  if (!text) return m.reply(`*Exemplo : ${prefix + command} FileName\n\nLihat list pesan dengan ${prefix}listmsg*`)
  let msgs = global.db.database
