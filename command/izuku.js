...
const wib = moment.tz('America/Sao_Paulo').format('HH:mm:ss')
const wita = moment.tz('America/Sao_Paulo').format("HH:mm:ss")
...
//𝐌𝐄𝐍𝐒𝐒𝐀𝐆𝐄𝐌 𝐍𝐎 𝐓𝐄𝐑𝐌𝐈𝐍𝐀𝐋
  ...
  if (command) {
  await izuku.sendPresenceUpdate('typing', m.chat)
  }
...
`
  ...
  break
  case 'menugrupo': case 'menugrp': {
  	let menugrupo = `╭「 *𝐆𝐑𝐔𝐏𝐎𝐒* 」
├ ${prefix}linkgrupo
├ ${prefix}revogarlink
├ ${prefix}remover
├ ${prefix}adicionar
...
└ ${prefix}ephemeral
  ...
  break
  case 'menuimagem': case 'menuimg': {
  	let menuimagem = `╭「 *𝐄𝐃𝐈𝐂𝐎𝐄𝐒 𝐈𝐌𝐆* 」
├ ${prefix}ktpmaker
├ ${prefix}candy
├ ${prefix}christmas
├ ${prefix}3dchristmas
...
└ ${prefix}leaves
  ...
  break
  case 'menufigurinhas': case 'menufg': {
  	let menufigurinhas = `╭「 *𝐅𝐈𝐆𝐔𝐑𝐈𝐍𝐇𝐀𝐒* 」
├ ${prefix}sticker
├ ${prefix}stickerwm
├ ${prefix}smeme
├ ${prefix}emojimix
...
└ ${prefix}handhold
  ...
  break
  case 'menualeatorio': case 'menualt': {
  	let menualeatorio = `╭「 *𝐀𝐋𝐄𝐀𝐓𝐎𝐑𝐈𝐎* 」
├ ${prefix}jodohku
├ ${prefix}jadian
├ ${prefix}bebangc
├ ${prefix}yatim
...
└ ${prefix}getcs
  ...
  break
  case 'menuoutros': case 'menuout': {
  	let menuoutros = `╭「 *𝐎𝐔𝐓𝐑𝐎𝐒* 」
├ ${prefix}apagar
├ ${prefix}owner
├ ${prefix}ping
├ ${prefix}listpc
...
case 'getcase': case 'getcs': {
  ...
  break
//Group Menu
case 'linkgrupo': case 'linkgrup': case 'linkgroup': case 'linkgroups': {
...
case 'removeradm': {
  ...
  break
case 'mudarnome': case 'setsubject': {
  if (!m.isGroup) return m.reply(mess.group)
  if (!isBotAdmins) return m.reply(mess.botAdmin)
  if (!isAdmins && !isOwner && !m.key.fromMe) return m.reply(mess.admin)
  if (!text) return m.reply(`*𝐄𝐱𝐞𝐦𝐩𝐥𝐨 : ${prefix + command} Texto*`)
...
case 'mudardesc': case 'setdesk': {
  ...
  break
case 'mudarftgrupo': case 'setppgrup': case 'setppgc': {
  if (!m.isGroup) return m.reply(mess.group)
  if (!isAdmins && !isOwner && !m.key.fromMe) return m.reply(mess.admin)
